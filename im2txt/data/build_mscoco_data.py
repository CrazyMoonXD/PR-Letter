...
def _to_sequence_example(image, vocab):
  ...
  feature_lists = tf.train.FeatureLists(feature_list={
      ...
      "image/npy": _float_feature_list(np.load(image.npyname))
  })
...
def _process_npy_files(thread_index, ranges, name, images, vocab,
                         num_shards):
...
def _process_dataset(name, images, vocab, num_shards):
  ...
  images = [ImageMetadata(image.image_id, image.npyname, [caption])
  ...
  for thread_index in xrange(len(ranges)):
    ...
    t = threading.Thread(target=_process_npy_files, args=args)
    t.start()
    threads.append(t)
...
def _load_and_process_metadata(captions_file, npy_dir):
  ...
  id_to_filename = [(x["id"], x["file_name"][:-3]+'npy') for x in caption_data["images"]]
  ...
  for image_id, base_filename in id_to_filename:
    npyname = os.path.join(npy_dir, base_filename)
    captions = [_process_caption(c) for c in id_to_captions[image_id]]
    image_metadata.append(ImageMetadata(image_id, npyname, captions))
