...
tf.flags.DEFINE_string("train_npy_dir", "/tmp/train2014/",
                       "Training image directory.")
tf.flags.DEFINE_string("val_image_dir", "/tmp/val2014",
                       "Validation image directory.")
tf.flags.DEFINE_string("val_npy_dir", "/tmp/val2014",
                       "Validation image directory.")
...
ImageMetadata = namedtuple("ImageMetadata",
                           ["image_id", "filename","npy_filename", "captions"])
...
def _to_sequence_example(image, decoder, vocab):
  ...
  feature_lists = tf.train.FeatureLists(feature_list={
      ...
      "image/name": _float_feature_list(np.load(image.npy_filename))
  })
...
#        print (sequence_example.SerializeToString())
        shard_counter += 1
        counter += 1
...
def _process_dataset(name, images, vocab, num_shards):
  ...
  images = [ImageMetadata(image.image_id, image.filename, image.npy_filename,[caption])
...
def _load_and_process_metadata(captions_file, image_dir,npy_dir):
  ...
  for image_id, base_filename in id_to_filename:
    ...
    npy_filename=os.path.join(npy_dir, base_filename[:-4]+'.npy')
    captions = [_process_caption(c) for c in id_to_captions[image_id]]
    image_metadata.append(ImageMetadata(image_id, filename,npy_filename, captions))
...
def main(unused_argv):
  ...
  mscoco_train_dataset = _load_and_process_metadata(FLAGS.train_captions_file,
                                                    FLAGS.train_image_dir,FLAGS.train_npy_dir)
  mscoco_val_dataset = _load_and_process_metadata(FLAGS.val_captions_file,
                                                  FLAGS.val_image_dir,FLAGS.val_npy_dir)
